...
difference() {
    ...
    translate([inch(-1), slot_inset, thickness - slot_depth]) {
        cube([inch(29), slot_width, slot_depth]);
    }
    // top through slot
    ...
    translate([slot_inset, slot_inset + slot_width, thickness - slot_depth]) {
        cube([slot_width, inch(24 + (21.0 / 32.0) + 0.05), slot_depth]);
    }
    // right stopped slot
    translate([inch(27) - (slot_inset + slot_width), slot_inset + slot_width, thickness - slot_depth]) {
        cube([slot_width, inch(24 + (21.0 / 32.0) + 0.05), slot_depth]);
    }
}
